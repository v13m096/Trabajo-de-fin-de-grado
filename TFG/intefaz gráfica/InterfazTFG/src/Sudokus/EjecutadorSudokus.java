...
public class EjecutadorSudokus {
	...
	public static String runFileWindows(String ruta,String[] prueba,int timeout)
	{
		...
		try
		{
			...
			if(processRun.waitFor(timeout, TimeUnit.MILLISECONDS)) {
                            try (BufferedReader brResult = new BufferedReader(new InputStreamReader(processRun.getInputStream()))) {
                                String outputRun;
                                int contador=0;
                                while( (outputRun = brResult.readLine())!=null)
                                {
                                 contador++;
                                 if (contador>4&&contador<5+(prueba.length/9)){
